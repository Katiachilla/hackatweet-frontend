 ...
 // création d'un nouvel utilisateur
    const handleRegister = () => {
      fetch('http://localhost:3000/users/signup', {
           ...
           router.push('/')   // redirige vers la page Home  //si data result est a true (fetch)
                })
      };
  ...
  return (
    <div className={styles.popover}>
      <span><FontAwesomeIcon icon={faXmark}   className={styles.crossIcon}/></span>
        ...
        <input onChange={(e) => setSignUpName(e.target.value)} value={signUpName}type="text" placeholder="Name"className={styles.inputStyle} />
        <input onChange={(e) => setSignUpUsername(e.target.value)} value={signUpUsername}type="text" placeholder="Username"className={styles.inputStyle}/>
        <input onChange={(e) => setSignUpPassword(e.target.value)} value={signUpPassword} type="password" placeholder="Password"className={styles.inputStyle}/>
        <button className={styles.buttonUp} onClick={handleRegister}>Sign up</button>
